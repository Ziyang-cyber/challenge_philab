...
def get_dataloaders(image_dir, label_dir, batch_size=16, train_size=0.8, num_workers=4, train_transform=None, val_transform=None):
    # Initialize the dataset (without specifying transformations)
    full_dataset = Sentinel2Dataset(image_dir=image_dir, label_dir=label_dir)
    ...
    # Apply transformations using the TransformWrapper
    train_dataset = TransformWrapper(Subset(full_dataset, train_indices), transform=train_transform)
    val_dataset = TransformWrapper(Subset(full_dataset, val_indices), transform=val_transform)
...
def visualize_samples(dataloader, num_samples=3):
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        for i in range(num_samples):
            img = images[i].numpy()[[3, 2, 1], :, :]  # Reorder channels for visualization
...
def model_prediction(model, input_dir, device):
    ...
    for img in input:
        ...
        # img = F.interpolate(img.unsqueeze(0), size=(512, 512), mode='nearest')
        img = img.to(device)
        with torch.no_grad():
            output = model(img[:, :7], img[:, 7:])
            _, preds = torch.max(output, 1)
            preds = preds.cpu().numpy().squeeze()
            preds = preds.astype(np.uint8)
            saved_dir = '/home/zhangz65/NASA_model/satlas/voting/Adamw_f1_90_10_12_bands_feature_fusion_e25/pred/'
...
if __name__ == "__main__":
    ...
    # Get DataLoaders
    train_loader, val_loader = get_dataloaders(image_dir, label_dir, batch_size=batch_size, train_size=1.0, num_workers=num_workers, train_transform=augment, val_transform=None)
    ...
    model = FloodPredictorHSL(path_weights="/home/zhangz65/NASA_model/satlas/voting/Adamw_f1_90_10_12_bands_feature_fusion_e25/phileo-precursor_v08_e025.pt")
    # model = FloodPredictorHSL(input_dim=7, output_dim=2, path_weights="phileo-precursor_v01_e027.pt")
    model = load_model(model, '/home/zhangz65/NASA_model/satlas/voting/Adamw_f1_90_10_12_bands_feature_fusion_e25/checkpoint/ckpt_best_f1.pth')
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    image_dir = '/home/zhangz65/NASA_model/phi_lab+model/phileo-precursor-model/Test Set Track 2/images'
